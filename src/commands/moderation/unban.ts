...
export default class extends Command<BotClient> {
  ...
  async action(message: Message, [user, reason]: [User, string]): Promise<any> {
    ...
    try {
      ...
      if (!bans.has(user.id)) return message.channel.send('This user was not already banned.');
      ...
      if (promptResult === PromptResult.TIMEOUT) return message.channel.send('Command timed out.');

      if (promptResult === PromptResult.FAILURE)
        return message.channel.send('Okay, aborting unban.');

      const unban: Message = (await message.channel.send(`unbanning **${user.tag}**`)) as Message;
      try {
