    ...
    async action(message: Message): Promise<any> {
        ...
        if (channel) {
            ...
            if (this.client.musicPlayer.streamDispatchers.has(guildId)) {
                const skipVoters: Collection<string, Map<string, GuildMember>> = this.client
                    .musicPlayer.playList.skipVoters;
                ...
                if (this.getVotePercentage(message.guild) >= minSkip) {
                    const current: Track = this.client.musicPlayer.playList.getCurrentTrack(
                        guildId
                    );
